...
def do_train(
    ...
    for epoch in range(0, max_epoch):
        ...
        for iteration, (batch_img, batch_att, batch_label) in enumerate(tr_dataloader):
            ...
            loss_dict = model(x=batch_img, att=batch_att, label=batch_label, seen_att=att_seen,att_all=att,score_local=score_local)


            Lreg = loss_dict['Reg_loss']
            Lcls = loss_dict['Cls_loss']
            Lcls_g = loss_dict['Clsg_loss']
            scale = loss_dict['scale']
            loss_dict.pop('scale')
            
            loss = lamd[1]*Lcls+ lamd[2]*Lreg + lamd[3]*Lcls_g
            # loss = lamd[1]*Lcls + lamd[3]*Lcls_g
            # loss = lamd[1]*Lcls+ lamd[2]*Lreg
            loss_dict_reduced = reduce_loss_dict(loss_dict)

            lreg = loss_dict_reduced['Reg_loss']
            lcls = loss_dict_reduced['Cls_loss']
            lcls_g = loss_dict_reduced['Clsg_loss']
            losses_reduced = lamd[1]*lcls + lamd[2]*lreg + lamd[3]*lcls_g
            # losses_reduced = lamd[1]*lcls + lamd[3]*lcls_g
            # losses_reduced = lamd[1]*lcls + lamd[2]*lreg
            optimizer.zero_grad()
            ...
            clsg_loss_epoch.append(lcls_g.item())
