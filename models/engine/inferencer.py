...
def cal_accuracy(model, dataloadr, att, test_id, device, att_all, score_local, bias=None):
    ...
    for iteration, (img, label) in enumerate(dataloadr):
        img = img.to(device)
        score = model(img, seen_att=att,att_all=att_all,score_local=score_local)
        scores.append(score)
        labels.append(label)
...
def eval(
    ...
    acc_zsl = cal_accuracy(model=model, dataloadr=tu_loader, att=att_unseen, test_id=test_id, device=device, att_all=att, score_local=score_local, bias=None)
    ...
    acc_gzsl_unseen = cal_accuracy(model=model, dataloadr=tu_loader, att=att, test_id=train_test_id, 
                                   device=device, att_all=att, score_local=score_local,bias=bias)
    acc_gzsl_seen = cal_accuracy(model=model, dataloadr=ts_loader, att=att, test_id=train_test_id, 
                                   device=device, att_all=att,score_local=score_local,bias=bias)
